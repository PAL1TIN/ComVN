...
typedef struct{
 ...
 int main(void) {
  ...
  do{
    ...
    printf("\n 1 - Cadastrar o número dos avioes");
    printf("\n 2 - Cadastrar o número de lugares disponíveis em cada aviao");
    printf("\n 3 - Reservar passagem");
   ...
   printf("\n Digite o numero do %d aviao: ",i);
    ...
    if(op == 2){
      for(i = 1; i <= 4; i++){
        printf("\n Digite o numero de lugares disponíveis no %d aviao: ",i);
        scanf("%d", &lug[i]);
      }
    }
    if(op == 3){
      ...
      for(i = 1; i <= 4; i++){
        if(posl > 60){
          printf("\n Nao ha mais lugares disponíveis para a reserva");
        }else{
          achou = 0;
            for(i =1; i <= 4; i++){
              if(num == avi[i]){
